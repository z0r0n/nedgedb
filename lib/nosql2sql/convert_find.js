...
function req_cols(list){
	...
	if(!cnt){
		return "SELECT * FROM PUB.";
	}
	else{
		ret_val = ret_val + " FROM PUB.";
		return ret_val;
	}
}
...
var convert = function(tablename,query,list,order){
	condition="";
	stack=[];
	var final_query = "";
	var where_exists="";
	final_query = req_cols(list)+tablename;
