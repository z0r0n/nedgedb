...
function set_attr(record){
	var ret_str="";
	var cnt=0;
	for (i in record){
		cnt++;
		ret_str = ret_str + i + "='" + record[i] +"'";
...
var convert = function(tablename,record,query,schema){
	condition="";
	stack=[];
	var final_query = "UPDATE "+schema+tablename+" SET "+set_attr(record);
